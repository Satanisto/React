...
export default class Slider extends Component {
  ...
  setSliderStyles() {
    const transition = (-100 / this.state.slides.length) * this.state.active;
  ...
  renderSlides() {
    const width = 100 / this.state.slides.length + "%";

    return this.state.slides.map((item, index) => (
      <div
        className="each-slide"
        key={index}
        style={{ backgroundImage: item.eachSlide, width: width }}
      />
    ));
  }
